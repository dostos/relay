...
7. **Escalation identity and inbox placement must be made target-independent.** This is the subtlest part of A, discovered in the storage layout:
   ...
   Naively retargeting delivery would therefore mint a *different* ID in a *different* directory for the same logical escalation — producing duplicate asks (exactly the "at most one unresolved ask per child handoff" property `DeliverPending` documents at `:672`). Resolution (**revised after implementation review — the first cut of this decision was unsafe and is recorded here so the mistake is not repeated**):
   - **Make identity target-independent.** Drop the parent id from `parentMessageID` so one logical escalation keeps one id wherever it lives, and catch replays with `FindMessage` (which already scans every inbox) rather than relying on an exclusive create inside a single directory.
     *Why the original plan failed:* it said "resolve the target before allocating, so no file moves are needed." But the target cannot be known before allocating, because liveness **is** the delivery outcome (decision 2) — you must have an envelope to attempt delivery with. The implementation therefore allocated against the intended manager and moved the file afterwards, which silently decoupled the dedup key from the storage location: the exclusive-create replay guard began checking an empty directory, and the non-exclusive rewrite at the destination **overwrote an already-resolved record**, erasing a recorded human decision and re-asking it. An adversarial review reproduced this end-to-end.
   - **Move the envelope only after a successful delivery.** Write it once in the intended manager's inbox; hand it to an ancestor only once that ancestor has actually received it. If nothing is reachable it stays with the intended manager, where `DeliverPending` retries it on reconnect — rather than being parked on a session that never saw it.
   - **Only attention envelopes fail over.** A routine `result`/`exit` receipt whose manager is asleep must never walk up and interrupt a human.
   - **Retry the intended manager before skipping it**, and only when an ancestor exists to fail over to. Otherwise a transient hiccup bypasses a manager that is genuinely live — a direct violation of decision 3's invariant.
