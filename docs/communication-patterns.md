...
| security | child → manager | explicit `permission_required` event or a live structured trust/login/tool decision surface; prose is never classified as a gate | parent envelope; recognized gates persist the decision subject and choices | exactly one wake per unchanged held decision; explicit resolve only (ack is refused) | `TestCommunicationLifecycleMatrix`, `TestQuotedGateProseDoesNotCreatePermissionEnvelope`, `TestChangedStructuredToolGateIsNotDeduplicated`, `TestExplicitGateApproveDeliversPendingGoalAndDenyCleansUp` |
| launch failure | launcher → initiating parent and direct caller | any rejected or partial managed start | durable attempt/handoff with stage-specific launch, delivery, presentation, cleanup, retry, event, and notice effects; compact direct response; one correlation-stable parent result envelope | never blank; parent delivery retries from its inbox even after the handoff terminalizes; secrets are redacted from notices | `TestPrePersistenceRejectionReturnsDurableAttempt`, `TestEventStreamAndVisualizationFailuresAreAtomic`, `TestManagedFailurePersistsParentEnvelopeAndDirectResponseOnce` |
...
| direct input | manager → immediate child | `parent send` | session edge; optional handoff event stream | default requires live handoff observability; `--delivery-only` makes composer-only delivery explicit | `TestManagedChildSendRequiresImmediateEdgeAndObservableEvents`, `TestManagedChildDeliveryOnlyMakesMissingEventsExplicit` |
